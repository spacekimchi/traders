...
#[get("")]
async fn get_calendar_root(tr: Data<tera::Tera>, _session: TypedSession, state: Data<AppState>) -> HttpResponse {
    ...
    match trades::get_trades_by_day_in_year(&state, this_year).await {
        Ok(trades_by_day) => {
            ...
            let _calendar_index_data = json!({
                "trades": trades_by_day,
                "mini_months": mini_months
            });
            match render_content(&RenderTemplateParams::new("calendar/index.html", &tr)) {
                Ok(calendar_template) => HttpResponse::Ok().body(calendar_template),
                Err(e) => HttpResponse::InternalServerError().body(err_500_template(&tr, e))
            }
        },
        Err(e) => HttpResponse::InternalServerError().body(err_500_template(&tr, e))
    }
